...
export const Navbar: React.FC<NavbarProps> = ({ currentView, onNavigate }) => {
  ...
  return (
    ...
    >
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <button
                ...
                className={`relative px-5 py-2.5 text-[10px] font-bold uppercase tracking-[0.2em] transition-all duration-300 group ${currentView === link.view ? 'text-[#036C42]' : 'text-gray-400 hover:text-[#036C42]'
                  }`}
              >
                {link.label}
                <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#47A4B5] transition-all duration-300 ${currentView === link.view ? 'opacity-100 scale-100' : 'opacity-0 scale-0 group-hover:opacity-50 group-hover:scale-100'
                  }`}></span>
              </button>
            ))}
            ...
            <button
              ...
              className={`px-6 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all duration-300 border ${currentView === 'contacts'
      ...
      <div className={`fixed inset-0 bg-[#036C42] z-[90] transition-all duration-700 lg:hidden flex flex-col items-center justify-center px-8 text-center ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none translate-y-10'
        }`}>
